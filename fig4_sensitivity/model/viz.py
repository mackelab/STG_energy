...
def single_neuron_sensitivity_bar(cum_grad, ylim, figsize, start=0, end=8, color='#3182bd'):
    fig, ax = plt.subplots(1, figsize=figsize)

    barlist = ax.bar(np.arange(len(cum_grad[0][start:end])), cum_grad[0][start:end], width=0.5)
    for i in range(8):
