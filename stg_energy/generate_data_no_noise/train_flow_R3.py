...
def train_flow(hyperparams):
    ...
    # Save data.
    with open(
        f"../../results/trained_neural_nets/inference/flow_{best_log_prob}_{seed}_no_noise.pickle",
        "wb",
    ) as handle:
        pickle.dump(posterior, handle, protocol=4)

    with open(
        f"../../results/trained_neural_nets/inference/posterior_11deg_no_noise.pickle",
        "wb",
    ) as handle:
        pickle.dump(posterior, handle, protocol=4)

    with open(
        f"../../results/trained_neural_nets/inference/inference_snpe_{best_log_prob}_{seed}_no_noise.pickle",
        "wb",
    ) as handle:
        pickle.dump(inference, handle, protocol=4)
    with open(
        f"../../results/trained_neural_nets/inference/hyperparams_{best_log_prob}_{seed}_no_noise.pickle",
        "wb",
    ) as handle:
