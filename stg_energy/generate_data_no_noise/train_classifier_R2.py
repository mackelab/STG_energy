...
path_to_data = (
    "simulation_data_Tube_MLslurm_cluster_no_noise/01_simulate_11deg_R2/data/all_"
)
...
print("shape of overall theta", theta.shape)
...
def compute_false_positives(batch_size, lr, dropout, hidden, blocks):
    ...
    with open(
        file_dir + f"optimized_network_estimator_no_noise.pickle",
        "rb",
    ) as handle:
        restriction_estimator = pickle.load(handle)
    ...
    file_dir = "../../results/trained_neural_nets/inference/"
    with open(
        file_dir + f"optimized_network_estimator_R2_no_noise.pickle",
        "wb",
    ) as handle:
        pickle.dump(restriction_estimator, handle, protocol=4)
    with open(
        file_dir + f"optimized_network_R2_no_noise.pickle",
        "wb",
    ) as handle:
