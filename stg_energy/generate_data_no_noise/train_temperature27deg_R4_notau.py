...
def train_flow(hyperparams):
    ...
    path_to_data = "simulation_data_Tube_MLslurm_cluster_no_noise/simulate_11deg_R3_predictives_at_27deg_notau/data/valid_"
    ...
    # Save data.
    with open(
        f"../../results/trained_neural_nets/inference/temp27_flow_notau_{best_log_prob}_{seed}.pickle",
        "wb",
    ) as handle:
        pickle.dump(posterior, handle, protocol=4)

    with open(
        f"../../results/trained_neural_nets/inference/posterior_27deg_no_noise.pickle",
        "wb",
    ) as handle:
        pickle.dump(posterior, handle, protocol=4)

    with open(
        f"../../results/trained_neural_nets/inference/temp27_inference_snpe_notau_{best_log_prob}_{seed}.pickle",
        "wb",
    ) as handle:
        pickle.dump(inference, handle, protocol=4)
    with open(
        f"../../results/trained_neural_nets/inference/temp27_hyperparams_notau_{best_log_prob}_{seed}.pickle",
        "wb",
    ) as handle:
