...
import stg_energy.experimental_data.experimental_data_utils as edu
import pyabf
...
class reader:
    ...
    def read_preparation(self, num, case="spike", subtract_PD=False):
        ...
        if case == "spike":
            ...
            if subtract_PD:
                self.subtract_PD_from_PY("828_042_2_PD_spikes_" + num + ".txt")
        elif case == "voltage":
...
class ABF_reader_016(reader):
    ...
    def subtract_PD_from_PY(self, file):
        print(1 / self.dt)
        self.PY_binary = edu.subtract_PDfromPY(
            self.PY_binary, self.PD_binary, vicinity=5, sampling_frequency=1 / self.dt
        )
        indizes = np.where(self.PY_binary == 1)[-1]
        self.PY_spike_times = indizes * self.dt
...
class ABF_reader_078(reader):
    ...
    def read_PD_spikes(self, file):
        self.PD_binary = edu.calc_spikes(self.pdn, threshold=0.05)
        indizes = np.where(self.PD_binary == 1)[-1]
        self.PD_spike_times = indizes * self.dt
