...
silent_thresh = (
    1  # minimum number of spikes per second for a neuron not to be considered silent
)
tonic_thresh = 30  # minimum number of spikes per second for a neuron to be considered tonically firing
burst_thresh = (
    0.5  # maximum percentage of single-spike bursts for a neuron to be considered
)
# bursting (and not simply spiking)
bl_thresh = (
    0  # minimum number of milliseconds for bursts not to be discounted as single spikes
)
spike_thresh = -10  # minimum voltage above which spikes are considered
ibi_threshold = (
    150  # maximum time between spikes in a given burst (ibi = inter-burst interval)
)
...
class PrinzExperimStats:
    ...
    def calc(self, reader):
        ...
        phase_gaps = [
            summ["phase_gaps"][neutypes[i], neutypes[j]] for i, j in ((0, 1), (1, 2))
        ]
...
class SpikeStats:
    ...
    def calc(self, repetition_list):
        ...
        for r in range(len(repetition_list)):
            ...
            ret[r] = np.asarray(
                [summ["cycle_period"], *burst_lengths, *ibi_lengths, *spike_counts]
            )
...
class CPGStats:
    def __init__(self, t_on, t_off, with_cycle_length=False, seed=None):
        ...
        self.labels = ["avg_burst_length_{}".format(nt) for nt in neutypes] + [
            "n_triphasic"
        ]
...
def analyse_neuron(t, spike_times):
    ...
    burst_end_spikes = np.append(
        np.where(np.diff(spike_times_ms) >= ibi_threshold)[0], num_spikes - 1
    )
    ...
    if len(burst_times) == 1:
        ...
        num_vals = 200
        # print(f"first {num_vals} cycle_lengths", cycle_lengths[:num_vals])
        # print("len", len(cycle_lengths))
        print("std of first ten: ", np.std(cycle_lengths[:num_vals]))
        # print("mean of first ten: ", np.mean(cycle_lengths[:num_vals]))
        # print("cycle_lengths[:10]", cycle_lengths[:num_vals])
