...
def buildCondCovMatrix(
    posterior, lims, samples=None, num_samples=10, max_dim=None, resolution=20
):
            ...
            for d2 in range(max_dim):
                if d1 < d2:
                    ...
                    cc = conditional_correlation(
                        p_image, lims[d1, 0], lims[d1, 1], lims[d2, 0], lims[d2, 1]
                    )
                    cc.calc_rhoXY()
...
def extractSpecificCondCorr(
    posterior, corrs, lims, samples=None, num_samples=10, max_dim=None, resolution=20
):
...
def eval_conditional_density(pdf, theta, lims, dim1, dim2, resolution=20, log=True):
    ...
    if dim1 == dim2:
        ...
        gbar_dim1 = np.linspace(
            lims[dim1, 0] + 1e-5, lims[dim1, 1] - 1e-5, resolution
        )  # todo get rid of 1e-5 (which is there to avoid being outside of the prior bounds)
...
class conditional_correlation:
    def __init__(
        self, cPDF, lower_bound_x, upper_bound_x, lower_bound_y, upper_bound_y
    ):
    ...
    def calc_rhoXY(self):
        self.calc_marginals()
        self.calc_EXY()
        self.EX = conditional_correlation.calc_E_1D(
            self.pdfX, self.lx, self.ux, self.resolution_x
        )
        self.EY = conditional_correlation.calc_E_1D(
            self.pdfY, self.ly, self.uy, self.resolution_y
        )
        self.CovXY = self.EXY - self.EX * self.EY
        self.VarX = conditional_correlation.calc_var1D(
            self.pdfX, self.lx, self.ux, self.resolution_x
        )
        self.VarY = conditional_correlation.calc_var1D(
            self.pdfY, self.ly, self.uy, self.resolution_y
        )
    ...
    def calc_EXY(self):
        x_matrix = np.tile(
            np.linspace(self.lx, self.ux, self.resolution_x), (self.resolution_y, 1)
        )
        y_matrix = np.tile(
            np.linspace(self.ly, self.uy, self.resolution_y), (self.resolution_x, 1)
        ).T
    ...
    def calc_marginals(self):
        ...
        self.pdfX = conditional_correlation.normalize_pdf_1D(
            self.pdfX, self.lx, self.ux, self.resolution_x
        )
        self.pdfY = conditional_correlation.normalize_pdf_1D(
            self.pdfY, self.ly, self.uy, self.resolution_y
        )
...
class conditional_mutual_information:
    ...
    def calc_HX(self):
        self.pdfX = self.normalize_pdf_1D(
            self.pdfX, self.lx, self.ux, self.resolution_x
        )
...
class D_KL_conditional_marginal:
    def __init__(
        self, pdf1, pdf2, lower_bound_x, upper_bound_x, lower_bound_y, upper_bound_y
    ):
        self.lx = lower_bound_x
        self.ux = upper_bound_x
        self.ly = lower_bound_y
